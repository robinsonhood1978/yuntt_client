<template>
	<view>
			...
			<view class="uni-flex uni-column">
				<view v-for="(item, index) in gallery.list" :key="index" class="item">
					<view class="uni-flex uni-row flex-middle round-edge bgf mg5">
						<navigator :url="'/pages/goods/index?id='+item.goods_id" hover-class="none" class="ml10 mt5">
						...
						<view class="uni-flex uni-column ml10 pb10 mr10 pt10">
							...
							<view class="uni-flex uni-row mt5 width-between">
								<navigator :url="'/pages/store/index?id='+item.store_id" open-type="redirect" hover-class="none"
									class="uni-flex uni-row flex-middle">
									<text class="f-13 f-gray line-clamp-1"
										style="max-width: 250rpx;">{{item.store_name}}</text>
									<uni-icons size="24" color="gray" type="arrowright"></uni-icons>
								</navigator>
								<text class="f-gray ml10">已抢{{item.sales||0}}件</text>
							</view>
							<view v-if="item.promotion.type == 'limitbuy'" class="uni-flex uni-row countdown mt5">
								<my-countdown :showDay="item.promotion.lefttime.d > 0" backgroundColor="#666"
									splitorColor="#999" color="#ffffff" :timestamp="item.promotion.timestamp">
								</my-countdown>
							</view>
							<view class="uni-flex uni-row mt10 width-between">
								<view class="uni-flex uni-row">
									<view v-if="item.promotion">
										<text class="f-red uni-bold">{{item.promotion.price|currency}}</text>
										<text class="ml10 f-gray f-12 del">{{item.price}}</text>
									</view>
									<view v-else class="f-red uni-bold">{{item.price|currency}}</view>
								</view>
								<navigator :url="'/pages/goods/index?id='+item.goods_id" hover-class="none"
									class="mybutton f-white">去抢购</navigator>
							</view>
						</view>
					</view>
				</view>
		...
		<view v-else class="uni-center">
			...
			<view class="uni-text-gray">暂无秒杀商品</view>
		</view>
