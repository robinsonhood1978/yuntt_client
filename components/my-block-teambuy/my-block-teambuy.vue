	...
	<view>
		<view class="gallery"
			...
			<view :class="['wraper pl5 pr5 pt10 pb10 bgf round-edge ml10 mr10']">
				<view :class="['title pb10 uni-center']">
					<view class="text">——<text class="uni-bold f-16 f-red ml10 mr10">拼团专区</text>——</view>
					<text class="f-gray">拼着买，更划算</text>
				</view>

				<view v-for="(item, index) in gallery" :key="index" class="round-edge bgf mt10 pb10 item">
					<navigator hover-class="none" :url="'/pages/goods/team?id='+item.goods_id"
						...
						<view class="extra width-surplus">
							...
							<view class="ml10 mt10">
								<text class="f-12 people">{{item.people}}人团</text>
								<text class="f-13 ml10">已拼{{item.sales||0}}件</text>
							</view>
							<view class="uni-flex uni-row mt10 pt10 flex-middle width-between">
								<view class="flex-middle">
									<text class="f-red f-16 uni-bold ml10">{{item.teamPrice|currency}}</text>
									<text class="del ml5 f-gray f-12">{{item.price|currency}}</text>
								</view>
								<view class="f-12 f-white mybutton">立即开团</view>
							</view>
						</view>
					</navigator>
				</view>
				<view class="uni-center f-gray mt10">
					<navigator url="/pages/teambuy/list" hover-class="none"><text class="f-13">更多拼团</text>
	...
	export default {
		...
		methods: {
			init() {
				...
				server.request('teambuy/list', params, function(res) {
					if (res.code == 0) {
